...
블록 레벨 요소는 부모 요소의 **전체 공간**을 차지하여 "블록"을 만든다. `<h1>~<h6>` `<ol>` `<ul>` `<li> ` `<p>` 태그 등이 블록 요소에 속한다.
...
인라인 레벨 요소는 콘텐츠의 흐름을 끊지 않고(줄바꿈X), 요소를 구성하는 **태그에 할당된 공간만 차지**한다. `<a>` `<em>` `<img>` `<span>`태그 등이 인라인 요소에 속한다.
